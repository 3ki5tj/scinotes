...
and
$\lim_{t\rightarrow 0} (\pmb\varphi(t) - \pmb\psi(t))/t = 0$
in some chart.
%
...
$$
TM_\mathbf{x} \rightarrow \mathbb R
\qquad
\pmb\xi \rightarrow \langle\pmb\xi, \pmb\xi \rangle.
$$
...
$\gamma:[t_0, t_1] \rightarrow M, t \rightarrow \mathbf x(t) \in M$,
then
$$
I(\gamma) = \int_{\mathbf x_0}^{\mathbf x_1}
...
\begin{defn*}
  The \emph{derivative} of a differentiable mapping $f: M \rightarrow N$
  at a point $\mathbf x \in M$ is the linear map of the tangent spaces
  $$
  f_{*\mathbf x}: TM_\mathbf{x} \rightarrow TN_{f(\mathbf x)},
  $$
...
are differentiable maps $G\times G \rightarrow G$
and $G \rightarrow G$.
%
\note{
The differentiable manifold defining the Lie group
...
\note{
  The identity element is the identity transformation.
  So the tangent space consists of
...
\begin{prob*}
  ...
  \answer{
    ...
    $$
    But this quantity is expected to be $\mathbf C_{ij} = \sum_k \epsilon_{ikj} c_k$,
    so
