...
Alternatively, try to evaluate this integral by the gamma (factorial) function:
\begin{equation}
  (s-1)!
=
  \Gamma(s)
\equiv
  \int_0^\infty z^{s-1} \, \exp(-z) \, dz,
  \label{eq:gammaDef}
\end{equation}
where $s$ is not necessarily an integer.
In our case, set $z = A \, x^2/2$.
}
...
\]
where $d\vct\uptheta^{(k)}$ denotes the angular element in $k$ dimensions,
and
\begin{align*}
  s   &= r \, \cos \theta_D, \\
  x_D &= r \, \sin \theta_D,
...
Thus, $I_n = I_{n-2} \, (n - 1) / n$, which leads to \eqref{eq:intsinn}.

Then, from \eqref{eq:drD_recursion}, we have
\begin{align}
    S_D(1)
  &= I_{D-2} \, S_{D-1}(1)
...
\end{align}
Discuss the cases of odd and even $D$,
and compare the results with \eqref{eq:SDoe}.

Note, as a by product of Eq. \eqref{eq:SD_recursion},
we can write $I_n$ as
\begin{equation}
  I_n
=
\sqrt{\pi} \, \Gamma\left( \tfrac{n+1}{2} \right) / \Gamma\left( \tfrac{n+2} 2 \right),
\label{eq:intsinn2}
\end{equation}
which unifies the even and odd cases.
}



\problem{A trignometry identity}
...
\begin{align}
  I_{m, n}
& \equiv \int_0^\pi \cos^m \theta \, \sin^n \theta \, d\theta
=
  \frac{(m-1)!! \, n!!} { (m+n)!! } I_{0, n},
  \label{eq:intcosmsinn}
\end{align}
where $I_{0, n} = I_n$
as given by Eq. \eqref{eq:intsinn} or \eqref{eq:intsinn2}.
...
&=
  ...
  \left(\frac n 2 \right)!
\end{align*}
Then convert the expression to double factorial.
}
...
\]
then evaluate the integral for each term by using Eq. \eqref{eq:intcos2ssinn}.
}
...
\problem{Three-dimensional Fourier transform of a spherical function}
...
\problem{$D$-dimensional Fourier transform of a spherical function}
