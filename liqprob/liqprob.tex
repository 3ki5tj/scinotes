...
=
\frac{ \Gamma\left( \tfrac 1 2 \right) \, \Gamma\left( \tfrac{n+1}{2} \right) }
     { \Gamma\left( \tfrac{n+2} 2 \right) },
\label{eq:intsinn2}
\end{equation}
...
\hint{
  Since $x! = x \, (x-1)!$, we have
\begin{align*}
  \left(\frac {m + n} 2 \right)!
&=
  \left(\frac {m + n} 2 \right)
  \left(\frac {m + n - 2} 2 \right)!
  = \cdots
\\
&=
  \left(\frac {n + m} 2 \right)
  \left(\frac {n + m - 2} 2 \right)
  \cdots
...
\end{align*}
%
Then convert the expressions to double factorials.
}
...
\]
then evaluate the integral for each term by using Eq. \eqref{eq:intcosmsinnbeta}.
}
