...
In this chapter,
we will review a few commonly-used mathematical tools.
%
The Gaussian integral is probably the most useful.
%
Bessel functions are somewhat advanced,
but we need them to Fourier transform
a spherical-symmetric function.



It is also good to know something about the gamma function, $\Gamma(z)$.
But if you have to skip it,
remember it is the extension of the factorial: $\Gamma(z) = (z-1)!$,
and $\Gamma(1/2) = \sqrt \pi$.
%
The beta function is less important,
it is like the inverse binomial coefficient.
...
The main purpose of this and the next problems
are to show the famous result:
$\Gamma(1/2) = \sqrt \pi$.
...
which then makes
Eq. \eqref{eq:gammaPi} look quite like
Eq. \eqref{eq:gammaDef}.
...
\]
Apply the above formula repeatedly to show Eq. \eqref{eq:intsinn}.
}
...
\]
which leads to Eq. \eqref{eq:intsinn}.
}
...
%
This relation,
unlike Eq. \eqref{eq:intcosmsinnb},
...
\problem{Moments of the Gaussian distribution}



Because of Eq. \eqref{eq:GaussianIntegral},
the Gaussian function can be normalized as a distribution:
%
\begin{equation}
  p(x) =
  \sqrt{ \frac{ A } { 2 \, \pi } }
  \exp\left( \frac 1 2 A x^2 \right).
...
1. By differentiating Eq. \eqref{eq:GaussianIntegral} with respect to $A$,
show that
%
\begin{equation}
  \langle x^2 \rangle
...
=
\frac{1}{A}.
\label{eq:GaussianVariance}
\end{equation}


2. As an extension of Eq. \eqref{eq:GaussianVariance},
show that
\begin{equation}
  \langle x^{2 n} \rangle
\equiv
\sqrt{ \frac { A } { 2 \, \pi } }
  \int_{-\infty}^{\infty}
    x^{ 2 \, n} \exp\left( - \frac{1}{2} A\,x^2 \right) \, dx
=
\frac{(2 \, n - 1)!!}{A^n}.
\end{equation}
...
1. Show the matrix form of Eq. \eqref{eq:GaussianIntegral}
\begin{equation}
  \int_{-\infty}^{\infty}
...
\hint{
  ...
  Show that the integral in Eq. \eqref{eq:GaussianIntegralMatrix}
  is equivalent to $n$ one-dimensional integrals,
  and evaluate them by Eq. \eqref{eq:GaussianIntegral}.
...
%
By differentiating Eq. \eqref{eq:GaussianIntegralMatrix} with respect to $A_{ij}$,
show that
%
\begin{equation}
...
2. By differentiating Eq. \eqref{eq:GaussianIntegralMatrixb} with respect to $b_i$ and $b_j$,
and then setting $\vct b = \vct 0$, show Eq. \eqref{eq:GaussianCorrelation}.



\problem{* A not-so-Gaussian integral}
...
\begin{align}
  ...
  \right]
  \, d x.
\label{eq:notSoGaussianIntegralDef}
\end{align}
%
The result is $\sqrt{2 \, \pi}/(2 \, e)$.
%
We will do it in different ways.
...
\begin{equation}
  ...
  \right]
  \, \frac{ d x } { x^2 }.
\label{eq:notSoGaussianIntegralInv}
\end{equation}
Then, by adding
Eqs. \eqref{eq:notSoGaussianIntegralDef}
and \eqref{eq:notSoGaussianIntegralInv},
show that
\begin{equation*}
  2 \, I
  =
  ...
  \, d \left(
    x - \frac 1 x
  \right).
%\label{eq:notSoGaussianIntegralAddInv}
\end{equation*}
Evaluate this by Gaussian integral.



2. In the second method,
we are going to do something like thermodynamic integration.
%
That is, integrating over a parameter.
%
First, we generalize
%
\begin{align}
  ...
  \right]
  \, d x.
\label{eq:notSoGaussianIntegralDefa}
\end{align}
%
Show by change of variable $y \equiv a/x$,
%
\begin{align*}
  \frac{d}{da}
  I(a)
  ...
  =-I(a).
\end{align*}
Thus, show that $I(a) = I(0) \, \exp(-a)$,
and also that $I(0) = \sqrt{\pi / 2}$
by Gaussian integral.

3. The third method
is most convenient if you remember how to compute residues.
%
First, show that
%
\begin{align*}
...
%
Then,
use this in Eq. \eqref{eq:notSoGaussianIntegralDef},
\begin{align*}
I
=
\frac { 1 } { \sqrt{ 2 \, \pi } }
...
\, d t,
\end{align*}
which can readily evaluated
by contour integral
from the residue at $t = i$.



4. The fourth method is the steepest descend.
%
This is an approximate method,
but it happens to yield the correct result in this case.
%
First, show that the maximum of $x^2 + 1/x^2$
is located at $x = 1$,
and at the vicinity,
we have
\[
...
Then,
use this in Eq. \eqref{eq:notSoGaussianIntegralDef},
and extend the domain of integration to
$(-\infty, +\infty)$.




\section{Spherical coordinates in high dimensions}
...
%
Usually, the dimensionality is usually one to three.
%
But occasionally,
it can be much larger
(e.g., recall the introduction of microcanonical ensemble
in which we have integrate over the $6N$-dimensional phase space.)
%
The first problem also serves
as a neat application of the Gaussian integral.
...
\end{align}
Then, show Eq. \eqref{eq:sphrcoordiff} by induction.
}

2. By integrating over the angular variables,
show that for a spherical symmetric function
the surface element is given by Eq. \eqref{eq:SD}.

\hint{
From Eq. \eqref{eq:drD_recursion}, we have
\begin{align}
    S_D(1)
  &= I_{D-2} \, S_{D-1}(1)
...
\]
by setting $\vct b = -i \vct k$ in Eq. \eqref{eq:GaussianIntegralMatrixb}.
%
...
%
Then, show that Eq. \eqref{eq:electrostaticenergy} can be rewritten as
\[
...
2. Differentiating Eq. \eqref{eq:Fexhnc} with respect to $\rho$,
show that it is identical to Eq. \eqref{eq:muexhnc}.

\hint{
  Use Eq. \eqref{eq:intdlogrhoc}.
}
