...
\hint{
  Let's call the integral on the left hand side as $I$.
  Show that
  \begin{align*}
    I^2
    &=
...
One can also evaluate this integral by the gamma function.
}
...
Show the matrix form of \eqref{eq:gaussianintegral}
\begin{equation}
  \int_{-\infty}^{\infty}
...
\begin{equation}
  \int_{-\infty}^{\infty}
  \exp\left( -\frac{1}{2} \vct{x}^T \vct{A} \vct{x} + \vct b^T \vct x \right) \, d\vct{x}
  =
  \frac{ (2\pi)^{n/2} }
  { |\vct A|^{1/2} }
...
Obtain the Fourier transform of
  $\exp\left( - \frac{1}{2} \vct x^T \vct A \vct x \right)$
  by setting $\vct b = -i \vct k$.
...
1. Argue the volume of a $D$-dimensional sphere of radius $R$ is proportional to
...
\problem{Fourier transform of a spherical function}
...
\[
  I(s) = s^{3D} \int f(s \vr_{12}) f(s \vr_{23}) f(s \vr_{34}) f(s \vr_{41})
  \, d\vr_2 \, d\vr_3 \, d\vr_4,
\]
is independent of $s$.
Then, differentiating $I(s)$ with respect to $s$.
}




\problem{Chemical potential and pressure}

Show that chemical potential $\mu$ and pressure $P$ are related by
\begin{equation}
  \rho \frac{ \partial \mu } { \partial \rho}
  =
  \frac{ \partial P } { \partial \rho }.
\end{equation}

\hint{
From $dE = T dS - P dV + \mu dN$, show that
\[
  -(\partial P/\partial N)_V = (\partial \mu/\partial V)_N.
\]
Then write $N$ and $V$ in terms of $\rho$.
}
...
1. Show that the excess chemical potential can be written as
\begin{equation}
  \muex
=
...
\end{equation}
%
where $U$ is potential energy function,
%
$\xi$ is called the ``charging'' parameter.
%
...
\begin{equation}
  \exp(-\beta \muex)
=
  \langle \exp[ -\beta \Delta U(\xi) ] \rangle,
\end{equation}
%
where $\Delta U$ is change of the potential energy after adding a particle into the system.
...
\problem{Lovett-Mou-Buff-Wertheim formula}
...
\end{align}
%
Show that differentiating $\beta P$ with respect to $\rho$
yields
\begin{align}
...
Show that the excess chemical potential in the HNC closure
can be found from\cite{morita1958, morita1959, morita1960, singer1985}
\begin{equation}
