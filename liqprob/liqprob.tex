...
\begin{align}
I_k
= \int \sin^k \theta \, d\theta
=
\begin{cases}
  2   \, (k-1)!! / k!!    &   \mbox{if $k$ is odd,} \\
  \pi \, (k-1)!! / k!!    &   \mbox{if $k$ is even.} \\
\end{cases}
\label{eq:intsink}
\end{align}
...
\begin{align*}
I_k
&= - \int \sin^{k-1} \theta \, d\cos\theta
= (k - 1) \, \int \sin^{k-2} \theta \, \cos^2\theta \, d\theta
\\
&= (k - 1) I_{k-2} - (k - 1) I_k,
\end{align*}
where we have used $\cos^2 \theta = 1 - \sin^2 \theta$ in the last step.
Thus, $I_k = I_{k-2} \, (k - 1) / k$, which leads to \eqref{eq:intsink}.
...
\problem{Arbitrary dimensional Fourier transform of a spherical function}
