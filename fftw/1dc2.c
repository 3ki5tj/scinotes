...
void cost2(int n, double *in, double *out)
{
  ...
  for (k = 0; k < n; k++)
    /* out[k] = 2 * sum_{j from 0 to n - 1} in[j] * cos(pi*(j+1/2)*k/n) */
    for (out[k] = 0, j = 0; j < n; j++)
      out[k] += 2 * in[j] * c[(2*j + 1) * k % (4*n)];
  free(c);
}
...
int main(void)
{
  ...
  for (i = 0; i < n; i++) in[i] = cos(2*M_PI*3*(i + .5) /n);
  ...
  q = fftw_plan_r2r_1d(n, out, in2, FFTW_REDFT01, FFTW_ESTIMATE);
  fftw_execute(q);
  for (i = 0; i < n; i++)
    printf("%6d %20.10f |  %20.10f\n", i, in[i], in2[i]/(2*n));
