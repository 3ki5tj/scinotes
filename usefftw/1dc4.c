...
void cost2(int n, double *in, double *out)
{
  ...
  c = calloc(8*n, sizeof(*c));
  for (j = 0; j < 8*n; j++) c[j] = cos(M_PI*j/(4*n));
  for (k = 0; k < n; k++)
    /* out[k] = 2 * sum_{j from 0 to n - 1} in[j] * cos(pi*(j+1/2)*(k+1/2)/n) */
    for (out[k] = 0, j = 0; j < n; j++)
      out[k] += 2 * in[j] * c[(2*j + 1) * (2*k + 1) % (8*n)];
  free(c);
}
...
int main(void)
{
  ...
  p = fftw_plan_r2r_1d(n, in, out, FFTW_REDFT11, FFTW_ESTIMATE);

  // for n = 3: in[0] = 1; in[1] = 2; in[2] = 4;
  for (i = 0; i < n; i++) in[i] = cos(2*M_PI*3.25*(i + .5)/n);
  ...
  q = fftw_plan_r2r_1d(n, out, in2, FFTW_REDFT11, FFTW_ESTIMATE);
  fftw_execute(q);
  for (i = 0; i < n; i++)
    printf("%6d %20.10f |  %20.10f\n", i, in[i], in2[i]/(2*n));
